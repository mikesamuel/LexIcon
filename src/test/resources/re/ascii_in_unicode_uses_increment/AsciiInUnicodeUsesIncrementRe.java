...
public final class AsciiInUnicodeUsesIncrementRe {
  private AsciiInUnicodeUsesIncrementRe() {
  }
  private static int match(CharSequence bufOpos, int idxOpos, int limit, int [] match) {
    ...
    bufPos = bufOpos;
    idxPos = idxOpos;
    if (! (idxPos + 10 < limit && Strings.substringMatches(bufPos, idxPos, "foo bar baz"))) {
      return -1;
    }
    idxPos += 11;
    match [0] = idxPos;
