...
public final class SimpleStringRe {
  private SimpleStringRe() {
  }
  private static int match(CharSequence bufOpos, int idxOpos, int limit, int [] match) {
    ...
    bufPos = bufOpos;
    idxPos = idxOpos;
    if (! (idxPos + 2 < limit && Strings.substringMatches(bufPos, idxPos, "foo"))) {
      return -1;
    }
    idxPos += 3;
    match [0] = idxPos;
